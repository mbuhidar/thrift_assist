...
# Configuration
FUZZ_THRESHOLD = 70  # Similarity threshold for phrase matching
...
def find_complete_phrases(phrase, text_lines, full_text, threshold=85):
    ...
    # Skip single-word phrases - we want complete phrases only
    if len(phrase_words) < 2:
        return matches
    
    # Search in full text first and try to map to line matches
    full_text_normalized = normalize_text_for_search(full_text)
    found_in_full_text = phrase_normalized in full_text_normalized
    ...
    for line in text_lines:
        ...
        if FUZZY_AVAILABLE and len(line_text_normalized) >= len(phrase_normalized):
            # Use enhanced matching that handles upside-down text
            ...
            if partial_sim >= 95:  # Very high threshold for partial matches
...
def main():
    ...
    # Custom search phrases for music CDs (based on what's actually in the image)
    search_terms = [
        'Maxi Priest',
        'Harry Connick',
        'Tracy Chapman',
    ...
    print("🎵 MUSIC PHRASE DETECTION AND ANNOTATION")
    print("=" * 50)
