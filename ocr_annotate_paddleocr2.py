...
def draw_ocr_boxes(image, result, search_terms=None, scale_x=1.0, scale_y=1.0,
    ...
    # PaddleOCR returns various formats: [bbox, (text, conf)] or dict format
    # or may have different nesting levels
    for item in result:
        total_detections += 1
        ...
        try:
            if isinstance(item, list) and len(item) >= 2:
                ...
                # Dictionary format
                bbox = item.get('bbox', item.get('boundingBox', []))
                text_info = (item.get('text', ''), item.get('confidence', 0.0))
            else:
                print(f"Skipping item with unexpected format: {item}")
                continue
                
            if len(bbox) < 4:
                print(f"Skipping item with insufficient bbox data: {item}")
                continue
        ...
        if len(bbox) == 4:
            ...
            pts = [tuple(map(int, (float(pt[0]) * scale_x,
                                   float(pt[1]) * scale_y))) for pt in bbox]
...
if __name__ == "__main__":
    ...
    # Handle PaddleOCR result format - it may return nested lists or dict
    if isinstance(result, list) and len(result) > 0:
        if isinstance(result[0], dict) and 'regions' in result[0]:
            result = result[0]['regions']
        elif isinstance(result[0], list):
            # PaddleOCR predict returns nested format
            result = result[0] if result else []
    
    print(f"PaddleOCR found {len(result)} text regions")
    ...
    plt.title("PaddleOCR Text Detection Results")
    plt.axis('off')
    plt.show()